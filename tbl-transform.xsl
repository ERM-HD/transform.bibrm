...
<xsl:stylesheet
    ...
    <xsl:template match="E_x0020_Journals_x0020_2012">
        <xsl:element name="bibrm:Item">
            ...
            <xsl:attribute name="bibrm:EISSN"><xsl:value-of select="concat('urn:issn:', E_x0020_ISSN)" /></xsl:attribute>
            <xsl:attribute name="bibrm:PISSN"><xsl:value-of select="concat('urn:issn:', P_x0020_ISSN)" /></xsl:attribute>
            <xsl:attribute name="bibrm:deliverdBy"><xsl:value-of select="Lieferant" /></xsl:attribute>
        </xsl:element>
    ...
    <xsl:template name="checkPackage">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($abo, 'Einzelabo')">
                <xsl:variable name="passed" select="'false'"/>
